...
# ------------------------------------------------------------------
# Math-aware page filter (now optional/informational)
# ------------------------------------------------------------------
def keep_page(text: str) -> bool:
    """Return True if page contains engineering math - currently not used."""
...
# ------------------------------------------------------------------
def normalize_vehicle_type(category: str) -> str:
    """Normalize folder name to standard vehicle type."""
    category_lower = category.lower().strip()

    # Map plural folder names to singular enum values
    mappings = {
        'drones': 'drone',
...
def process_file(file_path: Path, category: str) -> Optional[Dict]:
    ...
    vehicle_type = normalize_vehicle_type(category)
    ...
    if file_path.suffix.lower() == ".pdf":
        ...
        try:
            ...
            for page_num in range(pages_to_read):
                ...
                # Keep all pages with content (removed restrictive filter)
                if text.strip():
...
def test_search():
    ...
    for q in queries:
        ...
        for hit in hits:
            console.print(f"  [cyan]{hit.metadata.get('vehicle_type', 'unknown')}[/cyan] {hit.metadata['source']}")
...
def build_design_graph(
    ...
    graph.add_node("validate_params", validate_and_assume_parameters)
    ...
    graph.add_edge("parse", "validate_params")
    graph.add_edge("validate_params", "search")
...
def main():
    ...
    if not clear and "--clear" not in args:
