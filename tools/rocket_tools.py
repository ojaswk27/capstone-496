...
from .common_tools import G, air_density, MU_EARTH, R_EARTH
...
def optimize_staging(
    ...
    For equal Isp and structural fraction, optimal staging
    uses equal delta-v per stage.
    
    Args:
        total_delta_v: Total required delta-v (m/s)
        payload_mass: Final payload mass (kg)
        num_stages: Number of stages
        isp_stages: Isp for each stage (s)
        structural_fractions: Structural fraction for each stage
    ...
    # Simple approach: distribute delta-v equally
    ...
    for i in range(num_stages - 1, -1, -1):
        ...
        eps = structural_fractions[i] if i < len(structural_fractions) else structural_fractions[-1]
        
        # Calculate propellant mass for this delta-v
        mass_ratio = math.exp(dv_per_stage / (isp * G0))
        
        # m_0 = m_f × MR, where m_f = m_s + m_payload
        # m_s = eps × m_p / (1-eps)
        # m_p = m_0 - m_f = m_f × (MR - 1)
        # Solving: m_f = m_payload / (1 - eps × (MR - 1) / MR)
        
        denominator = 1 - eps * (mass_ratio - 1) / mass_ratio
        if denominator <= 0:
            # Stage is not feasible
            continue
            
        m_final = current_payload / denominator
        m_prop = m_final * (mass_ratio - 1)
        m_struct = eps * m_prop / (1 - eps)
        
        # Estimate thrust (T/W = 1.5 typical)
        total_stage_mass = m_prop + m_struct + current_payload
        thrust = 1.5 * total_stage_mass * G0
        
        stage = analyze_stage(
            propellant_mass=m_prop,
            structural_fraction=eps,
            isp=isp,
            thrust=thrust,
        ...
        # This stage becomes payload for next (lower) stage
        current_payload = m_prop + m_struct + current_payload
...
def delta_v_to_orbit(
    target_altitude: float,
    launch_latitude: float = 28.5
) -> float:
    """
