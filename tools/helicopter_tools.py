...
def calculate_disk_loading(thrust: float, disk_area: float) -> float:
    """
...
def calculate_solidity(num_blades: int, chord: float, radius: float) -> float:
    """
...
def calculate_tip_speed(rpm: float, radius: float) -> float:
    """
...
def calculate_induced_velocity(
    thrust: float, disk_area: float, altitude: float = 0
) -> float:
    """
...
def calculate_advance_ratio(forward_speed: float, tip_speed: float) -> float:
    """
...
def calculate_tail_rotor_thrust(main_rotor_torque: float, tail_arm: float) -> float:
    """
...
def calculate_autorotation_descent_rate(
    gross_weight: float, rotor: RotorResult, altitude: float = 0
) -> float:
    """
...
def calculate_autorotation_index(rotor: RotorResult, gross_weight: float) -> float:
    """
...
def design_helicopter(
    payload_kg: float, range_km: float, cruise_speed_kmh: float, is_manned: bool = None
) -> HelicopterDesignResult:
    """
    Complete helicopter preliminary design with scale awareness.
    """
    cruise_speed = cruise_speed_kmh / 3.6  # m/s

    # Auto-detect if manned
    if is_manned is None:
        is_manned = payload_kg > 100

    # Scale-dependent parameters
    if payload_kg < 50:  # Small UAV
        payload_fraction = 0.30
        empty_fraction = 0.50
        crew_weight = 0
        disk_loading_target = 200  # Lower for small
    elif payload_kg < 200:  # Light helicopter
        payload_fraction = 0.25
        empty_fraction = 0.55
        crew_weight = 80 if is_manned else 0
        disk_loading_target = 300
    else:  # Medium/heavy
        payload_fraction = 0.20
        empty_fraction = 0.60
        crew_weight = 160 if is_manned else 0
        disk_loading_target = 400

    gross_weight = payload_kg / payload_fraction

    # Weight iteration
    for iteration in range(10):
        rotor = design_rotor(gross_weight, disk_loading_target=disk_loading_target)
        hover = calculate_hover_power(gross_weight, rotor)

        flat_plate_area = 0.02 * gross_weight**0.67
        ...
        # SFC depends on engine type (piston vs turbine)
        if gross_weight < 300:  # Piston for small
            sfc = 0.45  # kg/kW-hr
        else:  # Turboshaft
            sfc = 0.30  # kg/kW-hr

        fuel_rate = sfc * (forward.power_required / 1000)  # kg/hr
        flight_time = range_km / cruise_speed_kmh
        fuel_weight = fuel_rate * flight_time * 1.1

        empty_weight = gross_weight * empty_fraction
        new_gw = empty_weight + payload_kg + fuel_weight + crew_weight

        if abs(new_gw - gross_weight) < 5:
            break

        gross_weight = 0.5 * gross_weight + 0.5 * new_gw

    tail_arm = rotor.diameter * 0.55
    tail_rotor_diameter = design_tail_rotor(hover.power_actual, rotor.rpm, tail_arm)
...
HELICOPTER_TOOLS = {
    ...
    "calculate_autorotation_descent_rate": {
        ...
        "parameters": {"gross_weight": "Weight (kg)", "rotor": "RotorResult"},
...
if __name__ == "__main__":
    ...
    heli = design_helicopter(payload_kg=400, range_km=500, cruise_speed_kmh=200)
