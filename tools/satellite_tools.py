...
from .common_tools import MU_EARTH, R_EARTH, G
...
def calculate_orbit_parameters(altitude: float, inclination: float = 0) -> OrbitResult:
    """
...
def calculate_ground_track_repeat(altitude: float, inclination: float = 98.0) -> int:
    """
...
def hohmann_transfer_delta_v(
    altitude_1: float, altitude_2: float
) -> Tuple[float, float]:
    """
...
def calculate_station_keeping_delta_v(
    altitude: float, years: float, orbit_type: str = "LEO"
) -> float:
    """
...
def size_solar_array(
    ...
    # Required area
    area = required_generation / (
        SOLAR_CONSTANT * efficiency * degradation * angle_factor
    )
...
def design_power_system(
    average_power: float, altitude: float, mission_years: float = 5
) -> PowerResult:
    """
    ...
    array_area = size_solar_array(
        average_power, eclipse_fraction=eclipse_fraction, mission_years=mission_years
    )

    # Size battery
    battery_capacity = size_battery(average_power, eclipse_duration)
...
def calculate_path_loss(frequency: float, distance: float) -> float:
    """
...
def design_satellite(
    payload_power: float, payload_mass: float, altitude: float, mission_years: float = 5
    ...
    # Mass estimation
    power_mass = power.solar_array_area * 8 + power.battery_capacity / 140  # kg
...
if __name__ == "__main__":
    ...
    sat = design_satellite(
        payload_power=50, payload_mass=20, altitude=500e3, mission_years=5
    )
